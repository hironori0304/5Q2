...
def main():
    ...
    if "attempts" not in st.session_state:
        st.session_state.attempts = 0  # 試行回数を初期化
    ...
    if uploaded_file is not None:
        try:
            ...
            for i, quiz in enumerate(st.session_state.current_quiz_data):
                st.markdown(f"**問題 {i + 1}**", unsafe_allow_html=True)
                st.markdown(f"<p style='margin-bottom: 0;'>{quiz['question']}</p>", unsafe_allow_html=True)

                options = quiz["options"]
                
                selected_option = st.radio("", options=options, index=None, key=f"question_{i}")
            ...
            if st.button("回答を送信"):
                score = 0
                incorrect_data = []
                st.session_state.attempts += 1  # 回答送信ボタンを押した回数をカウント
                ...
                if len(st.session_state.current_quiz_data) == 0:
                    st.success(f"すべての問題に正解しました！ 全問正解になるまで {st.session_state.attempts} 回挑戦しました。")
                    st.session_state.attempts = 0  # 全問正解後に試行回数をリセット
